...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        # nn.init.normal_(self.w_vs.weight, mean=0, std=np.sqrt(2.0 / (d_v + d_v)))
    ...
    def forward(self, q, k, v):
        ...
        f_k=torch.zeros(sz_b,self.filternum,dimen_k).cuda()
        f_v=torch.zeros(sz_b,self.filternum,dimen_v).cuda()
        ...
        # residual = q
        # print ('q1',q)
        # print ('q1 norm',torch.norm(q[0][0]))
        # print ('k1',k.shape, k)
        # print ('k1 norm',torch.norm(k[0][0]))
        # print ('v1',v)

        # q = self.w_qs(q).view(sz_b, len_q, n_head, d_k)
        # k = self.w_ks(f_k).view(sz_b, len_k, n_head, d_k)
        # v = self.w_vs(f_v).view(sz_b, len_v, n_head, d_v)
        ...
        # print ('attn out',attn)
        # print ('attn out sum',torch.sum(attn))
