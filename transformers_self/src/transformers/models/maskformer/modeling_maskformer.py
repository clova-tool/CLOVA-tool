...
class MaskFormerLoss(nn.Module):
    ...
    def forward(
        ...
        # print ('classification loss is reduced')
...
class MaskFormerTransformerModule(nn.Module):
    ...
    def __init__(self, in_features: int, config: MaskFormerConfig):
        ...
        self.prompt_embeddings = nn.Parameter(torch.zeros(1, 256, GLOBAL_TOKEN_H*GLOBAL_TOKEN_W)) ###  (1, num_tokens, prompt_dim) new added

    def init_prompt(self):
        self.prompt_embeddings.data.normal_()
    ...
    def forward(
        ...
        if use_prompt==False:
            ...
            image_features=image_features.view(batch_size, num_channels, height * width)
            image_features=torch.cat([image_features,torch.zeros(1, 256, GLOBAL_TOKEN_H*GLOBAL_TOKEN_W).to(image_features.device)],dim=2)
            if image_features.shape[2] % height ==0:
                image_features=image_features.view(batch_size, num_channels, height, -1)
            else:
                d= int (image_features.shape[2] / height)
                extra=height*(d+1)-GLOBAL_TOKEN_H*GLOBAL_TOKEN_W
                zero_prompt=torch.zeros(1, 256, extra).to(image_features.device)
                image_features=torch.cat([image_features,zero_prompt],dim=2)
                image_features=image_features.view(batch_size, num_channels, height, -1)
        else:
            if outer_prompt==None:
                batch_size, num_channels, height, width = image_features.shape
                # print ('size', batch_size, num_channels, height, width)
                image_features=image_features.view(batch_size, num_channels, height * width)
                # print ('image_features-1',image_features.shape)
                image_features=torch.cat([image_features,self.prompt_embeddings],dim=2)
                ...
                batch_size, num_channels, height, width = image_features.shape
                image_features=image_features.view(batch_size, num_channels, height * width)
                image_features=torch.cat([image_features,outer_prompt],dim=2)
                if image_features.shape[2] % height ==0:
                    image_features=image_features.view(batch_size, num_channels, height, -1)
                else:
                    d= int (image_features.shape[2] / height)
                    ...
                    image_features=image_features.view(batch_size, num_channels, height, -1)
        # print ('')
        # image_features=image_features.view(batch_size, num_channels, (height+GLOBAL_TOKEN_H), (width+GLOBAL_TOKEN_W))
        ####added by zhigao####
        ...
        # print ('object_queries',object_queries.shape)
...
class MaskFormerModel(MaskFormerPreTrainedModel):
    ...
    def init_prompt(self):
        self.transformer_module.init_prompt()
    ...
    def forward(
        ...
        transformer_module_output = self.transformer_module(image_features, output_hidden_states, output_attentions, use_prompt=use_prompt, outer_prompt=outer_prompt)
...
class MaskFormerForInstanceSegmentation(MaskFormerPreTrainedModel):
    ...
    def feature_extractor(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        image_features, pixel_embeddings = self.model.feature_extractor(
            pixel_values,
            pixel_mask,
            output_hidden_states=output_hidden_states or self.config.use_auxiliary_loss,
            return_dict=return_dict,
