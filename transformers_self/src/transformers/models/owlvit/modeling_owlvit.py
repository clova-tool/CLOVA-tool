...
from transformers_self.src.transformers.modeling_utils import PreTrainedModel
...
from transformers_self.src.transformers.models.owlvit.configuration_owlvit import OwlViTConfig, OwlViTTextConfig, OwlViTVisionConfig
...
class OwlViTVisionTransformer(nn.Module):
    def __init__(self, config: OwlViTVisionConfig):
        ...
        # self.linear = nn.Linear(2, 4)  ## For test, not used!!!!!!!!!!!!!
        # self.prompt_dropout = torch.nn.Dropout(0.1) ## new added For use
        total_d_layer = 3
        self.prompt_embeddings = nn.Parameter(torch.zeros(1, GLOBAL_TOKEN, 768)) ###  (1, num_tokens, prompt_dim) new added
        
    
    def init_prompt_embedding(self):
        # import math
        # from functools import reduce
        # from operator import mul
        # from itertools import repeat
        # val = math.sqrt(6. / float(3 * reduce(mul, tuple(repeat(32, 2)), 1) + 768))  # noqa
        # nn.init.uniform_(self.prompt_embeddings.data, -val, val)
        self.prompt_embeddings.data.zero_()
    ...
    def forward(
        ...
        hidden_states = self.embeddings(pixel_values) ### [1, 577, 768]
        hidden_states = self.pre_layernorm(hidden_states)
        ### Note there is a class embedding in the begin.
        if prompt_use:
            if outer_prompt == None:
                prompt = self.prompt_embeddings.expand(len(hidden_states), -1, -1)
                hidden_states = torch.cat((hidden_states[:,:1,:], hidden_states[:,1:,:], prompt), dim=1)
            else:
                prompt= outer_prompt.expand(hidden_states.shape[0], self.prompt_embeddings.shape[1],self.prompt_embeddings.shape[2])
                hidden_states=torch.cat((hidden_states[:,:1,:], hidden_states[:,1:,:], prompt), dim=1)
        else:
            if outer_prompt == 'ori':
                # print("false ori")
                ...
                # print("false none")
                prompt=torch.zeros(hidden_states.shape[0],self.prompt_embeddings.shape[1],self.prompt_embeddings.shape[2]).cuda()
                hidden_states=torch.cat((hidden_states[:,:1,:], hidden_states[:,1:,:], prompt), dim=1)
        ###
        #### add soft prompt here.
        encoder_outputs = self.encoder(
            inputs_embeds=hidden_states,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
...
class OwlViTModel(OwlViTPreTrainedModel):
    ...
    def forward(
        ...
        text_embeds = self.text_projection(text_embeds) #[4, 512]
        image_embeds = vision_outputs[1]
        image_embeds = self.visual_projection(image_embeds) # [1, 512]
...
class OwlViTBoxPredictionHead(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor) -> torch.FloatTensor:
        output = self.dense0(image_features) #[1, 576, 768]
        output = self.gelu(output)
        output = self.dense1(output) #[1, 576, 768]
        output = self.gelu(output)
        output = self.dense2(output) #[1, 576, 768]
        return output
...
class OwlViTForObjectDetection(OwlViTPreTrainedModel):
    ...
    def image_text_embedder(
        ...
        class_token_out = torch.broadcast_to(image_embeds[:, :1, :], new_size) ## [1, 577, 768]

        # Merge image embedding with class tokens
        image_embeds = image_embeds[:, 1:, :] * class_token_out
        image_embeds = self.layer_norm(image_embeds) ## [1, 576. 768]

        # Resize to [batch_size, num_patches, num_patches, hidden_size]
        if outer_prompt == 'ori' and prompt_use == False:
            new_size = (
            ...
            image_embeds = image_embeds.reshape(new_size) # [1, 24, 24, 768] ### new added
        else:
            new_size = (
            image_embeds.shape[0],
            int(np.sqrt(image_embeds.shape[1] - GLOBAL_TOKEN)),
            int(np.sqrt(image_embeds.shape[1] - GLOBAL_TOKEN)),
            image_embeds.shape[-1],
            )
            image_embeds = image_embeds[:,:-GLOBAL_TOKEN,:].reshape(new_size) # [1, 24, 24, 768] ### new added
    ...
    def forward(
        ...
        # Predict object classes [batch_size, num_patches, num_queries+1]
        (pred_logits, class_embeds) = self.class_predictor(image_feats, query_embeds, query_mask) ## image_feats [1, 576, 768]  query_embeds [1,4,512] query_mask [1,4]
        #[1, 576, 4] [1, 576, 512]
        # Predict object boxes
        pred_boxes = self.box_predictor(image_feats, feature_map) ## imag_feat [1, 576, 768] [1, 24, 24, 768]
        # pred_boxes [1, 576, 4]
        if not return_dict:
            output = (
                pred_logits,
